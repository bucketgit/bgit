...
import (
	...
	internalcli "github.com/bucketgit/bgit/internal/cli"
...
func ConfigureRuntime(options RuntimeOptions) {
	version = options.Version
	embeddedChangelog = options.Changelog
}
...
func RunExecutable(executable string, args []string, stdin io.Reader, stdout, stderr io.Writer) error {
	if filepath.Base(executable) == "git-remote-bgit" {
		return remoteHelperCommand(args, stdin, stdout, stderr)
	}
...
func Run(args []string, stdin io.Reader, stdout, stderr io.Writer) error {
	...
	intent := internalcli.ParseIntent(rest)
	cmd := intent.Command
	cmdArgs := intent.Args
	if intent.Version {
		return versionCommand(stdout)
...
func localProfileSelection(value string) (string, string) {
	return internalconfig.LocalSelection(value)
}

func globalLocalProfileForSelection(global internalconfig.Global, profileName, regionName string) (internalconfig.LocalProfile, internalconfig.ProfileRegion, bool) {
	...
	for _, profile := range global.LocalProfiles {
		...
		return profile, internalconfig.ProfileRegion{Name: regionName}, true
	}
	return internalconfig.LocalProfile{}, internalconfig.ProfileRegion{}, false
}
...
func parseGlobalFlags(args []string) (config, []string, error) {
	...
	parsed, rest, err := internalcli.ParseGlobalOptions(args, internalcli.GlobalOptionDefaults{
		Branch:          defaultBranch,
		Auth:            firstNonEmpty(envAuth, defaultAuthMode),
		Profile:         envConfiguration,
		AuthExplicit:    envAuth != "",
		ProfileExplicit: envConfiguration != "",
	})
	if err != nil {
		return config{}, nil, err
	}
	cfg := config{
		provider: parsed.Provider, bucket: parsed.Bucket, prefix: parsed.Prefix,
		branch: parsed.Branch, region: parsed.Region, auth: parsed.Auth,
		gcloudConfiguration: parsed.Profile, identity: parsed.Identity,
		authExplicit: parsed.AuthExplicit, authFlagExplicit: parsed.AuthFlagExplicit,
		gcloudConfigurationExplicit: parsed.ProfileExplicit, versionRequested: parsed.VersionRequested,
	}
	return cfg, rest, nil
}
...
func parseRepoURI(raw string) (config, string, error) {
	parsed, name, err := internalconfig.ParseRepositoryURI(raw)
	if err != nil {
		return config{}, "", err
	}
	return config{provider: parsed.Provider, bucket: parsed.Bucket, prefix: parsed.Prefix, branch: parsed.Branch, origin: parsed.Origin}, name, nil
}
...
func gcloudCommand(configuration string, args ...string) *exec.Cmd {
	return internalsetup.GCloudCommand(context.Background(), configuration, args...)
}

func usage(w io.Writer) error {
	return internalcli.Usage(w)
}

func usageWithBanner(w io.Writer) error {
	return internalcli.UsageWithBanner(w, bannerVersion())
}
...
func remoteCommand(args []string, stdout io.Writer) error {
	...
	return errors.New("supported remote commands: remote add origin gs://...|s3://... or remote set-url origin gs://...|s3://...") //lint:ignore ST1005 CLI metavariables intentionally end in ellipses.
}
...
func recordCreatedCloudProfile(provider, profile, region string, stdout io.Writer) error {
	...
	case "gcs":
		...
		global = upsertGlobalGCPProfile(global, internalconfig.GCPProfile{
			...
			ProjectID: projectID,
			Account:   account,
			Regions:   []internalconfig.ProfileRegion{{Name: region}},
		})
	case "s3":
...
func unsupportedCommand(cmd string) error {
	//lint:ignore ST1005 Preserve the established CLI error shown by Git passthrough failures.
