...
func TestBrokerInitWritesBrokerGitConfig(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		var req protocol.RepositoryRequest
...
func TestLocalBrokerStorageCompatibleRequiresSameCloudBucket(t *testing.T) {
	requested := protocol.Repository{Provider: "s3", Bucket: "123456789012-demo", Profile: "default", Region: "us-east-1"}
	existing := protocol.Repository{Provider: "s3", Bucket: "123456789012-demo", Profile: "default", Region: "us-east-1"}
	...
	fileRepo := protocol.Repository{Provider: "file", Bucket: "file://demo.git"}
...
func TestBoardCommandCreatesAndMovesStories(t *testing.T) {
	var createReq protocol.IssueRequest
	var moveReq protocol.IssueRequest
...
func TestBoardCommandEditsAndArchivesStories(t *testing.T) {
	var updateReq protocol.IssueRequest
	var archiveReq protocol.IssueRequest
	target, server, requests := setupBrokerCommandTestRepo(t, func(w http.ResponseWriter, r *http.Request) {
		switch r.URL.Path {
		case "/issues/update":
...
func TestBoardCommandPrioritizesStory(t *testing.T) {
	var reorderReq protocol.IssueRequest
	target, server, requests := setupBrokerCommandTestRepo(t, func(w http.ResponseWriter, r *http.Request) {
...
func TestBoardCommandListsArchivedStoriesSeparately(t *testing.T) {
	var listReq protocol.IssueRequest
	target, server, _ := setupBrokerCommandTestRepo(t, func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path != "/issues/list" {
			t.Fatalf("unexpected path %s", r.URL.Path)
		}
...
func TestBrokerUpgradeTargetUsesCurrentRepoProfile(t *testing.T) {
	global := internalconfig.Global{GCPProfiles: []internalconfig.GCPProfile{{
		Name: "work",
		Regions: []internalconfig.ProfileRegion{{
			Name:      "europe-west1",
...
func TestBrokerUpgradeTargetFallsBackToBrokerURL(t *testing.T) {
	global := internalconfig.Global{AWSProfiles: []internalconfig.AWSProfile{{
		Name: "prod",
		Regions: []internalconfig.ProfileRegion{{
			Name:      "us-east-1",
...
func TestInviteUserPreservesTeamScopedRepo(t *testing.T) {
	var got protocol.OwnerTransferRequest
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
...
func TestPrintBrokerUsersUsesReadableColumns(t *testing.T) {
	var stdout bytes.Buffer
	printBrokerUsers(&stdout, []protocol.UserInfo{{
		ID:         "u_owner",
		Username:   "owner",
		BrokerRole: "owner",
...
func TestBrokerInitInteractiveIdentityOnlyUpdatesRepoConfig(t *testing.T) {
	...
	if err := writeGlobalConfig(configPath, internalconfig.Global{
		Version:  globalConfigVersion,
		Identity: internalconfig.Identity{Name: "Global User", Email: "global@example.com"},
		GCPProfiles: []internalconfig.GCPProfile{{
			Name: "work",
			Regions: []internalconfig.ProfileRegion{{
				Name:      "europe-west1",
...
func TestBrokerInitInteractiveSelectsTeamThenRepository(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/repos/get":
			var req protocol.RepositoryAdminRequest
			if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
				t.Fatal(err)
			}
...
func TestInitDialogInitialStateUsesRepoThenGlobalIdentity(t *testing.T) {
	...
	initial := initDialogInitialState(target, internalconfig.Global{Identity: internalconfig.Identity{Name: "Global User", Email: "global@example.com"}}, "", "")
	...
	fresh := initDialogInitialState(filepath.Join(root, "fresh"), internalconfig.Global{Identity: internalconfig.Identity{Name: "Global User", Email: "global@example.com"}}, "new.git", "")
...
func TestAdminRepoCreateUsesCreateEndpointAndTeam(t *testing.T) {
	var got protocol.RepositoryAdminRequest
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
...
func TestBrokerAdminProtectAndPRCommandsUseBroker(t *testing.T) {
	target, server, requests := setupBrokerCommandTestRepo(t, func(w http.ResponseWriter, r *http.Request) {
		...
		case "/prs/create":
			var req protocol.PullRequestRequest
			if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
				t.Fatal(err)
			}
...
func TestImportGitHubKeysConfirmsAndStoresSource(t *testing.T) {
	var addReq protocol.KeyRequest
	target, server, _ := setupBrokerCommandTestRepo(t, func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("content-type", "application/json")
