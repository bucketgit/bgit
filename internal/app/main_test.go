...
func TestMain(m *testing.M) {
	...
	embeddedChangelog = string(changelog)
	brokerAssets = assetFileSystem{FS: os.DirFS("../..")}
	os.Exit(m.Run())
}
...
func TestLocalBrokerIssuesSupportBoardOperations(t *testing.T) {
	root := t.TempDir()
	server := &localBrokerServer{root: root, objectRoot: filepath.Join(root, "objects")}
	repo := server.physicalRepo(protocol.Repository{Provider: "file", Bucket: "file://demo.git", Prefix: "demo.git", Logical: "demo.git"})
	state := localbroker.RepositoryState{
		Repo: repo,
		Keys: []protocol.Key{{User: "owner", Role: "owner", PublicKey: "test-key"}},
		Refs: map[string]string{},
	}
	if err := server.saveRepo(state); err != nil {
		t.Fatal(err)
	}
	out, status, err := server.localPost("/issues/list", protocol.IssueRequest{Repo: repo, Type: "story"})
	...
	out, status, err = server.localPost("/issues/create", protocol.IssueRequest{Repo: repo, Type: "story", Title: "Local board works", Body: "Local board works", Lane: "backlog"})
	if err != nil || status != 200 {
	...
	if _, status, err = server.localPost("/issues/move", protocol.IssueRequest{Repo: repo, ID: createResp.Issue.ID, Lane: "doing"}); err != nil || status != 200 {
		t.Fatalf("move status=%d err=%v", status, err)
	}
	if _, status, err = server.localPost("/issues/assign", protocol.IssueRequest{Repo: repo, ID: createResp.Issue.ID, Assignee: "owner"}); err != nil || status != 200 {
		t.Fatalf("assign status=%d err=%v", status, err)
	}
	if _, status, err = server.localPost("/issues/archive", protocol.IssueRequest{Repo: repo, ID: createResp.Issue.ID, Archived: true}); err != nil || status != 200 {
	...
	out, status, err = server.localPost("/issues/list", protocol.IssueRequest{Repo: repo, Type: "story", IncludeArchived: true})
	if err != nil || status != 200 {
...
func TestLocalBrokerReorderNormalizesDenseStoryOrder(t *testing.T) {
	root := t.TempDir()
	server := &localBrokerServer{root: root, objectRoot: filepath.Join(root, "objects")}
	repo := server.physicalRepo(protocol.Repository{Provider: "file", Bucket: "file://ordered.git", Prefix: "ordered.git", Logical: "ordered.git"})
	state := localbroker.RepositoryState{
		Repo: repo,
		Keys: []protocol.Key{{User: "owner", Role: "owner", PublicKey: "test-key"}},
		Refs: map[string]string{},
	...
	for _, title := range []string{"one", "two", "three", "four"} {
		if out, status, err := server.localPost("/issues/create", protocol.IssueRequest{Repo: repo, Type: "story", Title: title, Lane: "doing"}); err != nil || status != 200 {
			t.Fatalf("create %q status=%d err=%v body=%s", title, status, err, out)
		}
	}
	out, status, err := server.localPost("/issues/reorder", protocol.IssueRequest{Repo: repo, ID: 4, Lane: "doing", Order: 3})
	if err != nil || status != 200 {
...
func TestNativeGitRepoBrokerPushDoesNotWriteRefThroughObjectStore(t *testing.T) {
	...
	var gotUpdate protocol.RefUpdateRequest
	updateCalls := 0
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch r.URL.Path {
		case "/auth/status":
...
func TestWebPullRequestCacheRendersPRTabAndPage(t *testing.T) {
	...
	if err := handler.writePullRequestCache([]protocol.PullRequest{{
		ID:        7,
		Title:     "Add docs",
...
func TestBrokerGitStoreReadsAndListsThroughBroker(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/objects/capability":
			var req protocol.ObjectCapabilityRequest
			if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
				t.Fatal(err)
			}
		...
		case "/objects/list":
			var req protocol.ObjectRequest
			if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
				t.Fatal(err)
			}
