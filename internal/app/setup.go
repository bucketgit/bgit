...
type setupSelection struct {
	Profiles                []internalsetup.Profile
	Keys                    []setupSSHKey
	IdentityName            string
...
func setupCommand(ctx context.Context, base config, args []string, stdin io.Reader, stdout io.Writer) error {
	...
	if opts.provider == "local" {
		...
		profiles = append(profiles, internalsetup.Profile{Provider: "local", Name: name, Region: firstNonEmpty(opts.region, "default")})
	}
...
selectAgain:
	for {
		...
		if strings.TrimSpace(selection.IdentityName) != "" || strings.TrimSpace(selection.IdentityEmail) != "" {
			global.Identity = internalconfig.Identity{
...
func setupSelectionIdentityChanged(selection setupSelection, cfg internalconfig.Global) bool {
...
func setupCreateProfileDefaults(profiles []internalsetup.Profile, opts setupOptions) map[string]string {
...
func configuredSetupBrokers(cfg internalconfig.Global) []setupConfiguredBroker {
...
func configuredSetupBrokerExists(cfg internalconfig.Global, provider, profile, region string) bool {
...
func profilesWithoutConfiguredExpansion(profiles []internalsetup.Profile) []internalsetup.Profile {
	seen := map[string]struct{}{}
	var out []internalsetup.Profile
	for _, profile := range profiles {
...
func setupProvisionSelectedProfile(base config, path, now string, profile internalsetup.Profile, opts setupOptions, publicKeys []string, global *internalconfig.Global, stdout io.Writer) error {
	_ = path
...
func discoverSetupProfiles(ctx context.Context) ([]internalsetup.Profile, error) {
	return internalsetup.DiscoverProfiles(ctx, internalsetup.ProfileDiscoveryOptions{ProbeTimeout: setupProbeTimeout})
}
...
func gcloudConfigValue(ctx context.Context, profile, key string) string {
	return internalsetup.GCloudConfigValue(ctx, profile, key, internalsetup.ProfileDiscoveryOptions{ProbeTimeout: setupProbeTimeout})
}

func parseAWSProfileFile(path string) []string {
	return internalsetup.ParseAWSProfileFile(path)
}

func configuredAWSProfileRegion(profile string) string {
	return internalsetup.ConfiguredAWSProfileRegion(profile, internalsetup.ProfileDiscoveryOptions{})
}

func awsProfileFileValue(path, profile, key string) string {
	return internalsetup.AWSProfileFileValue(path, profile, key)
}
...
func resolveGCPSetupRegion(profile internalsetup.Profile, explicitRegion string, interactive bool, stdin io.Reader, stdout io.Writer) (string, error) {
...
func resolveGCPSetupRegionsWithRaw(profile internalsetup.Profile, explicitRegion string, interactive bool, stdin io.Reader, rawInput io.Reader, stdout io.Writer) ([]string, error) {
...
func resolveAWSSetupRegion(ctx context.Context, profile internalsetup.Profile, explicitRegion string, interactive bool, stdin io.Reader, stdout io.Writer) (string, error) {
...
func resolveAWSSetupRegionsWithRaw(ctx context.Context, profile internalsetup.Profile, explicitRegion string, interactive bool, stdin io.Reader, rawInput io.Reader, stdout io.Writer) ([]string, error) {
	if err := requireSetupCLI("aws", "AWS"); err != nil {
		return nil, err
	}
	if strings.TrimSpace(explicitRegion) != "" {
...
func markConfiguredSetupProfiles(profiles []internalsetup.Profile, cfg internalconfig.Global) []internalsetup.Profile {
...
func configuredSetupProfileRegions(regions []internalconfig.ProfileRegion) []string {
	var out []string
	for _, region := range regions {
		if strings.TrimSpace(region.BrokerURL) == "" {
			continue
...
func setupRegionInitialSelection(profile internalsetup.Profile) []string {
...
func filterSetupProfiles(profiles []internalsetup.Profile, provider string, names []string, explicitRegion string) []internalsetup.Profile {
...
func setupProfileNameSelected(profile internalsetup.Profile, names map[string]struct{}) bool {
	candidates := []string{
		profile.Name,
...
func dedupeSetupProfiles(profiles []internalsetup.Profile) []internalsetup.Profile {
	seen := map[string]struct{}{}
	var out []internalsetup.Profile
	for _, profile := range profiles {
...
func discoverSetupSSHKeys(opts setupSSHKeyOptions) ([]setupSSHKey, error) {
	...
	for _, path := range opts.Paths {
		...
		keys = append(keys, parseSetupSSHKeys(string(data), path)...)
	}
	if !opts.NoAgent {
...
func setupBrokerUserActionChoices(user protocol.UserInfo) []setupChoice {
...
func runSetupBrokerUserAction(cfg config, broker setupConfiguredBroker, user protocol.UserInfo, action string, reader *bufio.Reader, rawInput io.Reader, stdout io.Writer) (string, error) {
	var out bytes.Buffer
	username := firstNonEmpty(user.Username, user.ID)
...
func setupRepoForTeamCreate(cfg config, value, teamID, teamName string) (protocol.Repository, error) {
...
func runSetupManagedTeamRepositoryAction(cfg config, teamID, teamName, repo, action string, reader *bufio.Reader, rawInput io.Reader, stdout io.Writer) (string, error) {
	...
	case "invite-user":
		...
		repoInfo, err := setupBrokerTeamRepoForAction(cfg, repo, teamID)
		if err != nil {
			return "", err
		}
		endpoints, err := brokerEndpointClient(brokerURL)
		...
		resp, err := endpoints.CreateRepositoryInvite(context.Background(), protocol.OwnerTransferRequest{Repo: repoInfo, BrokerURL: brokerURL, User: user, Role: role})
		if err != nil {
			return "", err
		}
	...
	case "cancel-invite":
		...
		user, ok, err := runSetupPendingRepoInviteSelect(cfg, repoInfo, reader, rawInput, stdout, setupBreadcrumb("Manage team", teamName, "Repositories", repoName, "Pending invite"))
		if err != nil || !ok {
			return "No changes made.", err
		}
		brokerURL, err := brokerURLFromConfigOrDiscovery(cfg)
		...
		if err := endpoints.CancelRepositoryInvite(context.Background(), protocol.OwnerTransferRequest{Repo: repoInfo, User: user}); err != nil {
			return "", err
		}
...
func runSetupRepoUserAssignmentWithRaw(cfg config, teamID, teamName, repo string, reader *bufio.Reader, rawInput io.Reader, stdout io.Writer) (string, error) {
	...
	for _, userID := range selected {
		...
		if err := endpoints.UpsertRepositoryUser(context.Background(), protocol.RepositoryAdminRequest{Repo: repoInfo, UserID: userID, Role: role}); err != nil {
			return "", err
		}
	}
...
func setupBrokerRepoTeamIDs(cfg config, repo, teamID string) ([]string, error) {
	...
	teams, err := endpoints.ListRepositoryTeams(context.Background(), protocol.RepositoryAdminRequest{Repo: repoInfo})
	if err != nil {
		return nil, err
	}
...
func setupRepoUsers(cfg config, repo, teamID string) ([]setupRepoUser, error) {
	...
	if grantErr == nil {
		...
		byName := map[string]protocol.UserInfo{}
...
func setupRepoKeys(cfg config, repo, teamID string) ([]protocol.Key, error) {
	brokerURL, err := brokerURLFromConfigOrDiscovery(cfg)
	if err != nil {
		return nil, err
	}
...
func setupRepoUserGrants(cfg config, repo, teamID string) ([]protocol.RepositoryUserGrant, error) {
	brokerURL, err := brokerURLFromConfigOrDiscovery(cfg)
	if err != nil {
		return nil, err
	}
	...
	users, err := endpoints.ListRepositoryUsers(context.Background(), protocol.RepositoryAdminRequest{Repo: repoInfo})
	if err != nil {
		return nil, err
	}
	return users, nil
...
func runSetupManagedTeamRepositoryUserAction(cfg config, teamID, teamName, repo, user, action string, reader *bufio.Reader, rawInput io.Reader, stdout io.Writer) (string, error) {
	...
	case "user-edit":
		...
		if hasGrant {
			if err := endpoints.UpsertRepositoryUser(context.Background(), protocol.RepositoryAdminRequest{Repo: repoForBroker(actionCfg), UserID: grant.UserID, User: firstNonEmpty(grant.User, grant.Username, user), Role: role}); err != nil {
				return "", err
			}
		}
		for _, key := range keys {
			if err := endpoints.RemoveKey(context.Background(), protocol.KeyRequest{Repo: repoForBroker(actionCfg), Key: key.PublicKey}); err != nil {
				return "", err
			}
		}
	...
	case "user-suspend", "user-remove":
		...
		if action == "user-remove" && hasGrant {
			if err := endpoints.RemoveRepositoryUser(context.Background(), protocol.RepositoryAdminRequest{Repo: repoForBroker(actionCfg), UserID: grant.UserID, User: firstNonEmpty(grant.User, grant.Username, user)}); err != nil {
				return "", err
			}
		}
		for _, key := range keys {
			request := protocol.KeyRequest{Repo: repoForBroker(actionCfg), Key: key.PublicKey}
			var err error
			if path == "/keys/remove" {
				err = endpoints.RemoveKey(context.Background(), request)
			} else {
				err = endpoints.SuspendKey(context.Background(), request, true)
			}
			if err != nil {
				return "", err
			}
...
func setupRepoKeysForUser(cfg config, repo, teamID, user string) ([]protocol.Key, error) {
	keys, err := setupRepoKeys(cfg, repo, teamID)
	if err != nil {
		return nil, err
	}
	var out []protocol.Key
	for _, key := range keys {
...
func setupRepoUserGrantForUser(cfg config, repo, teamID, user string) (protocol.RepositoryUserGrant, bool, error) {
	grants, err := setupRepoUserGrants(cfg, repo, teamID)
	if err != nil {
		return protocol.RepositoryUserGrant{}, false, err
	}
	...
	return protocol.RepositoryUserGrant{}, false, nil
}

func runSetupPendingRepoInviteSelect(cfg config, repo protocol.Repository, reader *bufio.Reader, rawInput io.Reader, stdout io.Writer, title string) (string, bool, error) {
...
func setupPendingRepoInviteChoices(cfg config, repo protocol.Repository) ([]setupChoice, error) {
	brokerURL, err := brokerURLFromConfigOrDiscovery(cfg)
	if err != nil {
		return nil, err
	}
	...
	invites, err := endpoints.ListRepositoryInvites(context.Background(), protocol.OwnerTransferRequest{Repo: repo})
	if err != nil {
		return nil, err
	}
	choices := make([]setupChoice, 0, len(invites))
...
func setupBrokerTeamRepoForAction(cfg config, repo, teamID string) (protocol.Repository, error) {
	actionCfg, err := setupBrokerRepoConfig(cfg, repo)
	if err != nil {
		return protocol.Repository{}, err
	}
...
func setupBrokerTeamInfo(cfg config, teamID string) (protocol.Team, error) {
	brokerURL, err := brokerURLFromConfigOrDiscovery(cfg)
	if err != nil {
		return protocol.Team{}, err
	}
	endpoints, err := brokerEndpointClient(brokerURL)
	if err != nil {
		return protocol.Team{}, err
	...
	return protocol.Team{}, fmt.Errorf("team %s not found", teamID)
}

func setupFormatTeamMembers(team protocol.Team) string {
	if len(team.Members) == 0 {
		return "No members."
	}
	var lines []string
...
func setupTeamDisplayName(fallback string, team protocol.Team) string {
...
func setupAvailableRepoInviteUserChoices(cfg config, repo, teamID string) ([]setupChoice, error) {
	...
	pending, err := setupPendingRepoInviteChoices(cfg, repoInfo)
	if err != nil {
		return nil, err
	}
...
func setupBrokerUsers(cfg config) ([]protocol.UserInfo, error) {
	brokerURL, err := brokerURLFromConfigOrDiscovery(cfg)
	if err != nil {
		return nil, err
	}
...
func setupBrokerUserByName(cfg config, username string) (protocol.UserInfo, bool, error) {
	users, err := setupBrokerUsers(cfg)
	if err != nil {
		return protocol.UserInfo{}, false, err
	}
	...
	return protocol.UserInfo{}, false, nil
}

func setupBrokerUserStatus(user protocol.UserInfo) string {
...
func setupBrokerUserChoicesFromUsers(users []protocol.UserInfo, exclude map[string]struct{}) []setupChoice {
	var choices []setupChoice
...
func setupBrokerRepos(cfg config) ([]protocol.RepositoryInfo, error) {
	brokerURL, err := brokerURLFromConfigOrDiscovery(cfg)
	if err != nil {
		return nil, err
	}
...
func (s setupDialogState) selection() setupSelection {
	var profiles []internalsetup.Profile
...
func setupDialogProviderOrder(profiles []internalsetup.Profile, createProviders []string) []string {
	seen := map[string]struct{}{}
	for _, profile := range profiles {
...
func setupProfileDisplayName(profile internalsetup.Profile) string {
	if profile.Existing && strings.TrimSpace(profile.Region) != "" {
...
func brokerUpsertOwners(brokerURL, bootstrapToken string, publicKeys []string) error {
	...
	return brokerPostJSONContextWithHeaders(context.Background(), brokerURL, "/owners/upsert", protocol.OwnerRequest{User: "owner", Role: "owner", PublicKeys: publicKeys}, nil, headers)
}

func brokerEnsureCoreTeam(brokerURL string) error {
	endpoints, clientErr := brokerEndpointClient(brokerURL)
	if clientErr != nil {
		return clientErr
	}
	_, err := endpoints.CreateTeam(context.Background(), protocol.RepositoryAdminRequest{TeamID: coreTeamID, Name: coreTeamName})
...
func upsertGlobalGCPProfile(cfg internalconfig.Global, profile internalconfig.GCPProfile) internalconfig.Global {
...
func upsertGlobalAWSProfile(cfg internalconfig.Global, profile internalconfig.AWSProfile) internalconfig.Global {
...
func upsertGlobalLocalProfile(cfg internalconfig.Global, profile internalconfig.LocalProfile) internalconfig.Global {
...
func mergeGlobalProfileRegions(existing, incoming []internalconfig.ProfileRegion) []internalconfig.ProfileRegion {
	out := append([]internalconfig.ProfileRegion{}, existing...)
