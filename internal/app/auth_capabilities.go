...
func whoamiAllCommand(ctx context.Context, cfg config, jsonOut bool, stdout io.Writer) error {
	...
	resp := protocol.RepositoriesMineResponse{Repos: repos}
	if jsonOut {
...
func brokerReposMineAllKeys(ctx context.Context, brokerURL string) ([]protocol.RepositoryMembership, error) {
	data := []byte(`{}`)
	headerSets := brokerSignatureHeaderSetsForBroker(brokerURL, "/repos/mine", data)
	if len(headerSets) == 0 {
		return nil, errors.New("no SSH agent keys available")
	}
	merged := map[string]protocol.RepositoryMembership{}
	var lastErr error
	for _, headers := range headerSets {
		var resp protocol.RepositoriesMineResponse
	...
	var repos []protocol.RepositoryMembership
...
func repoMembershipWarnings(repos []protocol.RepositoryMembership) []string {
	byRepo := map[string][]protocol.RepositoryMembership{}
...
func brokerWhoami(ctx context.Context, cfg config, refresh bool) (protocol.AuthStatus, error) {
	if !refresh {
	...
	if err != nil {
		return protocol.AuthStatus{}, err
	}
	status, err := endpoints.AuthStatus(ctx, protocol.AuthStatusRequest{Repo: repoForBroker(cfg)})
	if err != nil {
		return protocol.AuthStatus{}, err
	}
...
func readWhoamiCache(brokerURL string) (protocol.AuthStatus, error) {
...
func writeWhoamiCache(brokerURL string, status protocol.AuthStatus) error {
	path, err := whoamiCachePath(brokerURL)
	if err != nil {
		return err
	}
