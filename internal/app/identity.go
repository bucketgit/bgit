...
const defaultBucketGitIdentityName = internalidentity.DefaultName

type identityEmailValidator struct{}

func (identityEmailValidator) MatchString(value string) bool {
	return internalidentity.ValidEmail(value)
}

var identityEmailPattern identityEmailValidator
...
func defaultBucketGitIdentityEmail() string {
	...
	return internalidentity.DefaultEmail(username)
}

func readGlobalIdentity() internalconfig.Identity {
	path, err := defaultGlobalConfigPath()
	if err != nil {
		return internalconfig.Identity{}
	}
...
func effectiveRepositoryIdentity(repo *localRepository) identityConfig {
	...
	value := internalidentity.Effective(name, email, defaultEmail)
	return identityConfig{Name: value.Name, Email: value.Email, UsesDefault: value.UsesDefault}
}
...
func readIdentityFields(reader *bufio.Reader, stdout io.Writer, currentName, currentEmail string) (string, string, error) {
	...
	if email != "" && !internalidentity.ValidEmail(email) {
...
func writeGlobalIdentity(name, email string) error {
	...
	cfg.Identity = internalconfig.Identity{Name: strings.TrimSpace(name), Email: strings.TrimSpace(email)}
