...
func TestWhoamiCommandWritesGlobalCache(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_ = json.NewEncoder(w).Encode(protocol.AuthStatus{
			BrokerVersion: "1.0.0-dev",
			Repo:          protocol.Repository{Provider: "gcs", Logical: "foo.git"},
			Identity:      protocol.Identity{User: "dennis", KeyFingerprint: "SHA256:test"},
			User:          "dennis",
			Role:          "admin",
...
func TestRepoMembershipWarningsShowAmbiguousKeys(t *testing.T) {
	warnings := repoMembershipWarnings([]protocol.RepositoryMembership{
...
func TestExplicitProfileSelectionAppliesToRepositoryDiscovery(t *testing.T) {
	...
	if err := writeGlobalConfig(path, internalconfig.Global{
