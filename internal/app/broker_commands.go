...
func brokerAdminCommandWithInput(cfg config, args []string, stdin io.Reader, stdout io.Writer) error {
	if len(args) == 0 {
		//lint:ignore ST1005 CLI usage errors intentionally include formatted guidance.
...
func brokerProvisionResolvedTarget(path string, target brokerProfile, global *internalconfig.Global, stdout io.Writer) (provisionedBroker, error) {
	cfg := config{
	...
	case "gcs":
		...
		*global = upsertGlobalGCPProfile(*global, internalconfig.GCPProfile{
			Name:           target.Name,
			ProjectID:      existing.ProjectID,
			Account:        existing.Account,
			ServiceAccount: serviceAccount,
			Regions: []internalconfig.ProfileRegion{{
				Name:          target.Region,
	...
	case "s3":
		...
		*global = upsertGlobalAWSProfile(*global, internalconfig.AWSProfile{
			Name:      target.Name,
...
func findGlobalGCPProfile(cfg internalconfig.Global, name string) internalconfig.GCPProfile {
	for _, profile := range cfg.GCPProfiles {
		if profile.Name == name {
			return profile
		}
	}
	return internalconfig.GCPProfile{Name: name}
}

func findGlobalAWSProfile(cfg internalconfig.Global, name string) internalconfig.AWSProfile {
	for _, profile := range cfg.AWSProfiles {
		if profile.Name == name {
			return profile
		}
	}
	return internalconfig.AWSProfile{Name: name}
}

func brokerUpgradeTargetForCurrentRepo(cfg config, global internalconfig.Global) (brokerProfile, error) {
...
func brokerUsersCommand(cfg config, args []string, stdout io.Writer) error {
	...
	if len(args) == 1 && args[0] == "list" {
		users, err := endpoints.ListUsers(context.Background(), protocol.RepositoryAdminRequest{})
		if err != nil {
			return err
		}
		printBrokerUsers(stdout, users)
		return nil
	}
	if len(args) == 2 && args[0] == "delete" {
		req := protocol.RepositoryAdminRequest{User: args[1]}
		if err := endpoints.DeleteUser(context.Background(), req); err != nil {
			return err
		}
	...
	if len(args) >= 2 && args[0] == "upsert" {
		req := protocol.RepositoryAdminRequest{User: args[1], BrokerRole: "user"}
		...
		user, err := endpoints.UpsertUser(context.Background(), req)
		if err != nil {
			return err
		}
...
func printBrokerUsers(stdout io.Writer, users []protocol.UserInfo) {
...
func brokerTeamsCommand(cfg config, args []string, stdout io.Writer) error {
	...
	if len(args) == 1 && args[0] == "list" {
		teams, err := endpoints.ListTeams(context.Background(), protocol.RepositoryAdminRequest{})
		if err != nil {
			return err
		}
	...
	if len(args) >= 2 && args[0] == "create" {
		req := protocol.RepositoryAdminRequest{Name: args[1]}
		team, err := endpoints.CreateTeam(context.Background(), req)
		if err != nil {
			return err
		}
	...
	if len(args) == 2 && args[0] == "delete" {
		req := protocol.RepositoryAdminRequest{TeamID: args[1]}
		if err := endpoints.DeleteTeam(context.Background(), req); err != nil {
			return err
		}
	...
	if len(args) >= 4 && args[0] == "member" && args[1] == "add" {
		req := protocol.RepositoryAdminRequest{TeamID: args[2], User: args[3], Role: "read"}
		...
		if err := endpoints.UpsertTeamMember(context.Background(), req); err != nil {
			return err
		}
	...
	if len(args) == 4 && args[0] == "member" && args[1] == "remove" {
		req := protocol.RepositoryAdminRequest{TeamID: args[2], User: args[3]}
		if err := endpoints.RemoveTeamMember(context.Background(), req); err != nil {
			return err
		}
	...
	if len(args) == 2 && args[0] == "repo" && args[1] == "list" {
		...
		cfgEndpoints, err := brokerEndpointClient(cfg.brokerURL)
		if err != nil {
			return err
		}
		teams, err := cfgEndpoints.ListRepositoryTeams(context.Background(), protocol.RepositoryAdminRequest{Repo: repoForBroker(cfg)})
		if err != nil {
			return err
		}
		for _, team := range teams {
	...
	if len(args) >= 4 && args[0] == "repo" && args[1] == "add" {
		...
		req := protocol.RepositoryAdminRequest{Repo: repoForBroker(cfg), TeamID: args[2], Role: role}
		cfgEndpoints, err := brokerEndpointClient(cfg.brokerURL)
		if err != nil {
			return err
		}
		if err := cfgEndpoints.UpsertRepositoryTeam(context.Background(), req); err != nil {
			return err
		}
	...
	if len(args) == 3 && args[0] == "repo" && args[1] == "remove" {
		...
		req := protocol.RepositoryAdminRequest{Repo: repoForBroker(cfg), TeamID: args[2]}
		cfgEndpoints, err := brokerEndpointClient(cfg.brokerURL)
		if err != nil {
			return err
		}
		if err := cfgEndpoints.RemoveRepositoryTeam(context.Background(), req); err != nil {
			return err
		}
...
func brokerAdminRepoCommand(cfg config, args []string, stdout io.Writer) error {
	...
	if args[0] == "list" {
		...
		repos, err := endpoints.ListRepositories(context.Background(), protocol.RepositoryAdminRequest{})
		if err != nil {
			return err
		}
	...
	endpoints, err := brokerEndpointClient(cfg.brokerURL)
	if err != nil {
		return err
	}
	switch args[0] {
	case "info":
		...
		resp, err := endpoints.RepositoryInfo(context.Background(), protocol.RepositoryAdminRequest{Repo: repoForBroker(cfg)})
		if err != nil {
			return err
		}
	...
	case "visibility":
		...
		req := protocol.RepositoryAdminRequest{Repo: repoForBroker(cfg), Visibility: args[1]}
		if err := endpoints.UpdateRepository(context.Background(), req); err != nil {
			return err
		}
	...
	case "readonly":
		...
		readOnly := args[1] == "on"
		req := protocol.RepositoryAdminRequest{Repo: repoForBroker(cfg), ReadOnly: &readOnly}
		if err := endpoints.UpdateRepository(context.Background(), req); err != nil {
			return err
		}
	...
	case "issues":
		...
		req := protocol.RepositoryAdminRequest{Repo: repoForBroker(cfg), IssuesEnabled: &issuesEnabled}
		if err := endpoints.UpdateRepository(context.Background(), req); err != nil {
			return err
		}
	...
	case "rename":
		...
		if err := endpoints.RenameRepository(context.Background(), protocol.RepositoryAdminRequest{Repo: repoForBroker(cfg), Logical: logical}); err != nil {
			return err
		}
	...
	case "delete":
		...
		if err := endpoints.DeleteRepository(context.Background(), protocol.RepositoryAdminRequest{Repo: repoForBroker(cfg)}); err != nil {
			return err
		}
...
func brokerAdminRepoCreateCommand(cfg config, args []string, stdout io.Writer) error {
	...
	repo.TeamName = team
	req := protocol.RepositoryAdminRequest{Repo: repo, Role: role}
	endpoints, err := brokerEndpointClient(brokerURL)
	if err != nil {
		return err
	}
	if _, err := endpoints.CreateRepository(context.Background(), req); err != nil {
		return err
	}
...
func janitorMembersReindex(cfg config, stdout io.Writer) error {
	...
	if err := endpoints.ReindexRepositoryMembers(context.Background(), protocol.KeyRequest{Repo: repoForBroker(cfg)}); err != nil {
		return err
	}
...
func brokerCloneWithEphemeralLocalBroker(opts brokerInitOptions, repoName string, stdout io.Writer) error {
	...
	defer managed.Close()
	global = upsertGlobalLocalProfile(global, internalconfig.LocalProfile{
		Name:      profile.Name,
		Root:      profile.Root,
		Autostart: true,
		Regions:   []internalconfig.ProfileRegion{region},
	})
	if err := writeGlobalConfig(path, global); err != nil {
		return err
	}
...
func startDefaultLocalBroker(global internalconfig.Global) (internalconfig.LocalProfile, internalconfig.ProfileRegion, *managedLocalBroker, error) {
	...
	if err != nil {
		return internalconfig.LocalProfile{}, internalconfig.ProfileRegion{}, nil, err
	}
...
func prepareLocalBrokerRepository(brokerURL, bootstrapToken string, repo protocol.Repository, keys []string) (protocol.Repository, error) {
	if len(keys) == 0 {
		return protocol.Repository{}, errors.New("local broker requires at least one local SSH public key")
	}
	...
	resp, err := endpoints.CreateRepository(context.Background(), protocol.RepositoryAdminRequest{Repo: repo, Role: "developer", User: "owner", PublicKeys: keys})
	if err == nil {
	...
	resp, getErr := endpoints.GetRepository(context.Background(), protocol.RepositoryRequest{Repo: repo})
	if getErr != nil {
		return protocol.Repository{}, err
	}
	if strings.TrimSpace(resp.Repo.Logical) != "" {
		if !localBrokerStorageCompatible(repo, resp.Repo) {
			return protocol.Repository{}, fmt.Errorf("repository %s already exists with %s storage; requested %s storage", resp.Repo.Logical, localBrokerStorageDescription(resp.Repo), localBrokerStorageDescription(repo))
		}
		return resp.Repo, nil
	}
	return repo, nil
...
func brokerInitWithLocalProfile(opts brokerInitOptions, repoName string, profile brokerProfile, global internalconfig.Global, path, identityName, identityEmail string, stdout io.Writer) error {
	...
	global = upsertGlobalLocalProfile(global, internalconfig.LocalProfile{
		...
		Autostart: true,
		Regions:   []internalconfig.ProfileRegion{localRegion},
	})
	if err := writeGlobalConfig(path, global); err != nil {
		return err
	}
...
func ensureDefaultLocalProfile(global internalconfig.Global) internalconfig.LocalProfile {
	...
	return internalconfig.LocalProfile{Name: "default", Root: root, Autostart: true}
}

func ensureDefaultLocalRegion(profile internalconfig.LocalProfile) internalconfig.ProfileRegion {
	...
	return internalconfig.ProfileRegion{Name: "default"}
}
...
func brokerRepoForAdminCreateTarget(cfg config, brokerURL, repoName, teamID string) (protocol.Repository, error) {
...
func localBrokerRepoForTarget(cfg config, repoName, teamID string) (protocol.Repository, error) {
...
func localBrokerCloudIdentityFromConfig(ctx context.Context, scheme, profile string) (string, error) {
	...
	if err != nil {
		if errors.Is(err, os.ErrNotExist) {
			global = internalconfig.Global{Version: globalConfigVersion}
		} else {
...
func localBrokerImportCloudIdentity(ctx context.Context, scheme, profile string, global internalconfig.Global, path string) (string, error) {
	profile = firstNonEmpty(strings.TrimSpace(profile), "default")
	if global.Version == 0 {
	...
	case "s3":
		...
		global = upsertGlobalAWSProfile(global, internalconfig.AWSProfile{
			...
			AccountID: accountID,
			ARN:       arn,
			Regions:   []internalconfig.ProfileRegion{{Name: firstNonEmpty(configuredAWSProfileRegion(profile), "us-east-1")}},
		})
		if err := writeGlobalConfig(path, global); err != nil {
			return "", err
		}
	...
	case "gs":
		...
		global = upsertGlobalGCPProfile(global, internalconfig.GCPProfile{
			...
			Regions:   []internalconfig.ProfileRegion{{Name: firstNonEmpty(gcloudConfigValue(ctx, profile, "run/region"), gcloudConfigValue(ctx, profile, "functions/region"), "us-central1")}},
		})
		if err := writeGlobalConfig(path, global); err != nil {
			return "", err
		}
...
func localBrokerStorageCompatible(requested, existing protocol.Repository) bool {
...
func normalizeLocalBrokerStorageRepo(repo protocol.Repository) protocol.Repository {
...
func localBrokerStorageDescription(repo protocol.Repository) string {
...
func brokerCloneWithLocalProfile(opts brokerInitOptions, repoName string, profile brokerProfile, stdout io.Writer) error {
	...
	req := protocol.RepositoryRequest{Repo: protocol.Repository{Logical: logical, TeamID: firstNonEmpty(profile.TeamID, coreTeamID)}}
	endpoints, err := brokerEndpointClient(managed.URL)
	if err != nil {
		return err
	}
	resp, err := endpoints.GetRepository(context.Background(), req)
	if err != nil {
		return err
	}
	if strings.TrimSpace(resp.Repo.Logical) == "" {
...
func brokerConfirmOwnershipTransferCommand(cfg config, args []string, stdout io.Writer) error {
	...
	repo := protocol.Repository{Provider: "gcs", Logical: logical, Origin: "git@" + defaultSSHHost + ":" + logical}
	endpoints, err := brokerEndpointClient(brokerURL)
	if err != nil {
		return err
	}
	resp, err := endpoints.ConfirmOwnerTransfer(context.Background(), protocol.OwnerTransferRequest{Repo: repo, BrokerURL: brokerURL})
	if err != nil {
		return err
	}
...
func brokerAcceptOwnershipTransferCommand(args []string, stdout io.Writer) error {
	...
	endpoints, err := brokerEndpointClient(payload.BrokerURL)
	if err != nil {
		return err
	}
	resp, err := endpoints.AcceptOwnerTransfer(context.Background(), protocol.OwnerTransferRequest{Repo: payload.Repo, Token: payload.Token, User: "owner"})
	if err != nil {
		return err
	}
...
func brokerCancelOwnershipTransferCommand(cfg config, args []string, stdout io.Writer) error {
	...
	if err := endpoints.CancelOwnerTransfer(context.Background(), protocol.OwnerTransferRequest{Repo: repo}); err != nil {
		return err
	}
...
func brokerInviteUserCommand(cfg config, args []string, stdout io.Writer) error {
	...
	resp, err := endpoints.CreateRepositoryInvite(context.Background(), protocol.OwnerTransferRequest{Repo: repo, BrokerURL: brokerURL, User: user, Role: role})
	if err != nil {
		return err
	}
...
func brokerAcceptInviteCommand(args []string, stdout io.Writer) error {
	...
	resp, err := endpoints.AcceptRepositoryInvite(context.Background(), protocol.OwnerTransferRequest{Repo: payload.Repo, Token: payload.Token})
	if err != nil {
		return err
	}
...
func brokerCancelInviteCommand(cfg config, args []string, stdout io.Writer) error {
	...
	if err := endpoints.CancelRepositoryInvite(context.Background(), protocol.OwnerTransferRequest{Repo: repo, User: user}); err != nil {
		return err
	}
...
func brokerInviteBrokerUserCommand(cfg config, args []string, stdout io.Writer) error {
	...
	resp, err := endpoints.CreateBrokerUserInvite(context.Background(), protocol.RepositoryAdminRequest{User: user, BrokerRole: role, BrokerURL: brokerURL})
	if err != nil {
		return err
	}
...
func brokerAcceptBrokerInviteCommand(args []string, stdout io.Writer) error {
	...
	resp, err := endpoints.AcceptBrokerUserInvite(context.Background(), protocol.RepositoryAdminRequest{User: payload.User, Token: payload.Token})
	if err != nil {
...
func brokerCancelBrokerInviteCommand(cfg config, args []string, stdout io.Writer) error {
	...
	if err := endpoints.CancelBrokerUserInvite(context.Background(), protocol.RepositoryAdminRequest{User: user}); err != nil {
		return err
	}
...
func brokerRepoForAdminTarget(cfg config, repoName, teamID string) (protocol.Repository, error) {
...
func brokerRepoForStorageTarget(cfg config, repoName, teamID string) (protocol.Repository, bool, error) {
	raw := strings.TrimSpace(repoName)
	scheme := ""
	switch {
	...
	logical, err := logicalRepoFromStorageTarget(raw)
	if err != nil {
		return protocol.Repository{}, true, err
	}
	local := cfg
	local.logicalRepo = logical
	local.prefix = ""
...
func brokerProtectionCommand(cfg config, args []string, stdout io.Writer) error {
	...
	switch args[0] {
	case "list":
		protections, err := endpoints.ListProtections(context.Background(), protocol.Protection{Repo: repoForBroker(cfg)})
		if err != nil {
			return err
		}
	...
	case "add":
		...
		req := protocol.Protection{Repo: repoForBroker(cfg), Ref: ref, RequirePR: true, AllowOverrides: allowOverrides}
		if err := endpoints.UpsertProtection(context.Background(), req); err != nil {
			return err
		}
	...
	case "remove":
		...
		req := protocol.Protection{Repo: repoForBroker(cfg), Ref: normalizeDestinationRef(args[1])}
		if err := endpoints.RemoveProtection(context.Background(), req); err != nil {
			return err
		}
...
func brokerAdminCICommand(cfg config, args []string, stdout io.Writer) error {
	...
	case "rotate-secret":
		...
		if err := endpoints.RotateCISecret(context.Background(), protocol.CIRequest{Repo: repoForBroker(cfg)}); err != nil {
			return err
		}
...
func ciCommand(args []string, stdin io.Reader, stdout io.Writer) error {
	...
	switch args[0] {
	case "list":
		runs, err := endpoints.ListCIRuns(context.Background(), protocol.CIRequest{Repo: repoForBroker(cfg)})
		if err != nil {
			return err
		}
	...
	case "view":
		...
		run, err := endpoints.GetCIRun(context.Background(), protocol.CIRequest{Repo: repoForBroker(cfg), ID: id})
		if err != nil {
			return err
		}
		fmt.Fprintf(stdout, "#%d %s\nprovider: %s\nref: %s\ncommit: %s\nconfig: %s\n", run.ID, firstNonEmpty(run.Status, "queued"), run.Provider, run.Ref, run.Commit, run.Config)
		if run.URL != "" {
	...
	case "logs":
		...
		logs, err := endpoints.CILogs(context.Background(), protocol.CIRequest{Repo: repoForBroker(cfg), ID: id})
		if err != nil {
			return err
		}
		resp.Run, resp.Logs = logs.Run, logs.Logs
		if resp.Logs != "" {
	...
	case "run":
		req := protocol.CIRequest{Repo: repoForBroker(cfg), Provider: defaultCIProvider(cfg)}
		for i := 1; i < len(args); i++ {
			arg := args[i]
			name, value, hasValue := strings.Cut(arg, "=")
			switch name {
		...
		run, err := endpoints.RunCI(context.Background(), req)
		if err != nil {
			return err
		}
		fmt.Fprintf(stdout, "queued CI run #%d for %s at %s\n", run.ID, shortRefName(run.Ref), shortHash(run.Commit))
		if run.Message != "" {
...
func issueCommand(args []string, stdin io.Reader, stdout io.Writer) error {
	...
	switch args[0] {
	case "list":
		issues, err := endpoints.ListIssues(context.Background(), protocol.IssueRequest{Repo: repoForBroker(cfg)})
		if err != nil {
			return err
		}
	...
	case "create":
		...
		issue, err := endpoints.CreateIssue(context.Background(), protocol.IssueRequest{Repo: repoForBroker(cfg), Title: title, Body: body})
		if err != nil {
			return err
		}
	...
	case "view":
		...
		issue, err := endpoints.GetIssue(context.Background(), protocol.IssueRequest{Repo: repoForBroker(cfg), ID: id})
		if err != nil {
			return err
		}
		fmt.Fprintf(stdout, "#%d %s\n%s\n\n%s\n", issue.ID, issue.Title, firstNonEmpty(issue.Status, "open"), issue.Body)
	...
	case "comment":
		...
		if err := endpoints.CommentIssue(context.Background(), protocol.IssueRequest{Repo: repoForBroker(cfg), ID: id, Comment: comment}); err != nil {
			return err
		}
	...
	case "close", "reopen":
		...
		request := protocol.IssueRequest{Repo: repoForBroker(cfg), ID: id}
		var actionErr error
		if args[0] == "reopen" {
			actionErr = endpoints.ReopenIssue(context.Background(), request)
		} else {
			actionErr = endpoints.CloseIssue(context.Background(), request)
		}
		if actionErr != nil {
			return actionErr
		}
...
func boardCommand(args []string, stdin io.Reader, stdout io.Writer) error {
	...
	case "list":
		...
		issues, err := endpoints.ListIssues(context.Background(), protocol.IssueRequest{Repo: repoForBroker(cfg), Type: "story", IncludeArchived: includeArchived})
		if err != nil {
			return err
		}
	...
	case "create":
		...
		issue, err := endpoints.CreateIssue(context.Background(), protocol.IssueRequest{Repo: repoForBroker(cfg), Type: "story", Title: storySummary(story), Body: story, Lane: "backlog"})
		if err != nil {
			return err
		}
		fmt.Fprintf(stdout, "created story %s\n", storyDisplayID(monogram, issue.ID))
		return nil
	case "edit":
		if len(args) < 3 {
		...
		if err := endpoints.UpdateIssue(context.Background(), protocol.IssueRequest{Repo: repoForBroker(cfg), ID: id, Type: "story", Title: storySummary(story), Body: story}); err != nil {
	...
	case "move":
		...
		if err := endpoints.MoveIssue(context.Background(), protocol.IssueRequest{Repo: repoForBroker(cfg), ID: id, Lane: lane}); err != nil {
			return err
		}
	...
	case "priority", "order", "reorder":
		...
		if err := endpoints.ReorderIssue(context.Background(), protocol.IssueRequest{Repo: repoForBroker(cfg), ID: id, Lane: lane, Order: order}); err != nil {
			return boardUpgradeError(err)
		}
		if lane == "" {
	...
	case "take":
		...
		if err := endpoints.TakeIssue(context.Background(), protocol.IssueRequest{Repo: repoForBroker(cfg), ID: id}); err != nil {
			return err
		}
	...
	case "assign":
		...
		if err := endpoints.AssignIssue(context.Background(), protocol.IssueRequest{Repo: repoForBroker(cfg), ID: id, Assignee: assignee}); err != nil {
			return err
		}
		if assignee == "" {
	...
	case "archive", "unarchive":
		...
		if err := endpoints.ArchiveIssue(context.Background(), protocol.IssueRequest{Repo: repoForBroker(cfg), ID: id, Archived: archived}); err != nil {
			return boardUpgradeError(err)
		}
		if archived {
...
func sortBoardStories(stories []protocol.Issue) {
...
func prCommand(args []string, stdin io.Reader, stdout io.Writer) error {
	...
	case "list":
		prs, err := endpoints.ListPullRequests(context.Background(), protocol.PullRequestRequest{Repo: repoForBroker(cfg)})
		if err != nil {
			return err
		}
		for _, pr := range prs {
	...
	case "close":
		...
		if _, err := endpoints.ClosePullRequest(context.Background(), protocol.PullRequestRequest{Repo: repoForBroker(cfg), ID: id}); err != nil {
			return err
		}
	...
	case "reopen":
		...
		if _, err := endpoints.ReopenPullRequest(context.Background(), protocol.PullRequestRequest{Repo: repoForBroker(cfg), ID: id}); err != nil {
			return err
		}
	...
	case "merge":
		...
		if _, err := endpoints.MergePullRequest(context.Background(), protocol.PullRequestRequest{Repo: repoForBroker(cfg), ID: id, Merge: true, DeleteBranch: deleteBranch}); err != nil {
			return err
		}
	...
	case "comment":
		...
		if _, err := endpoints.CommentPullRequest(context.Background(), protocol.PullRequestRequest{Repo: repoForBroker(cfg), ID: id, Comment: comment}); err != nil {
			return err
		}
	...
	case "approve", "reject":
		...
		if _, err := endpoints.ReviewPullRequest(context.Background(), protocol.PullRequestRequest{Repo: repoForBroker(cfg), ID: id, Review: review, Comment: comment}); err != nil {
			return err
		}
...
func prCreateCommand(cfg config, args []string, stdout io.Writer) error {
	pr := protocol.PullRequest{Target: "refs/heads/main"}
	for i := 0; i < len(args); i++ {
		arg := args[i]
		name, value, hasValue := strings.Cut(arg, "=")
		switch name {
	...
	created, err := endpoints.CreatePullRequest(context.Background(), protocol.PullRequestRequest{Repo: repoForBroker(cfg), PR: pr})
	if err != nil {
		return err
	}
...
func brokerGetPullRequest(cfg config, args []string) (protocol.PullRequest, error) {
	...
	pr, err := endpoints.GetPullRequest(context.Background(), protocol.PullRequestRequest{Repo: repoForBroker(cfg), ID: id})
	if err != nil {
		return protocol.PullRequest{}, err
	}
	return pr, nil
...
func resolveBrokerTeamName(brokerURL, teamName string) (string, error) {
	...
	team, err := endpoints.ResolveTeam(context.Background(), protocol.RepositoryAdminRequest{Name: teamName})
	if err != nil {
		return "", err
	}
...
func loadGlobalConfigForInit(path string) (internalconfig.Global, string, error) {
	var err error
	if path == "" {
		path, err = defaultGlobalConfigPath()
		if err != nil {
			return internalconfig.Global{}, "", err
		}
	}
	cfg, err := readGlobalConfig(path)
	if err != nil {
		if errors.Is(err, os.ErrNotExist) {
			return internalconfig.Global{Version: globalConfigVersion}, path, nil
		}
		return internalconfig.Global{}, path, err
	}
	return cfg, path, nil
}

func brokerProfilesFromGlobalConfig(cfg internalconfig.Global) []brokerProfile {
...
func brokerInitProfilesWithLocal(profiles []brokerProfile) []brokerProfile {
	...
	out = append(out, brokerInitLocalProfile(internalconfig.Global{}))
	return out
}

func brokerInitLocalProfile(global internalconfig.Global) brokerProfile {
	profile := ensureDefaultLocalProfile(global)
	region := ensureDefaultLocalRegion(profile)
	return brokerProfile{
		Provider:      "local",
...
func brokerInitTeamChoices(profile brokerProfile) ([]setupChoice, error) {
	endpoints, endpointErr := brokerEndpointClient(profile.BrokerURL)
	if endpointErr != nil {
		return nil, endpointErr
	}
	teamsResponse, err := endpoints.ListTeams(context.Background(), protocol.RepositoryAdminRequest{})
	if err != nil {
	...
	choices := make([]setupChoice, 0, len(teamsResponse))
...
func normalizeLogicalRepoName(name string) (string, error) {
	return internalconfig.NormalizeLogicalRepository(name)
}
...
func initBrokerWorktree(target, repoName string, profile brokerProfile, identityName, identityEmail string, stdout io.Writer) error {
	...
	repo := protocol.Repository{Provider: profile.Provider, Logical: repoName, Prefix: repoName, Origin: fmt.Sprintf("git@%s:%s", defaultSSHHost, repoName), TeamID: profile.TeamID}
	return initBrokerWorktreeWithLocalRepo(target, repo, profile, identityName, identityEmail, stdout)
}

func initBrokerWorktreeWithLocalRepo(target string, repo protocol.Repository, profile brokerProfile, identityName, identityEmail string, stdout io.Writer) error {
...
func brokerRequireExistingLogicalRepo(brokerURL, provider, logicalRepo, teamID string) error {
	...
	_, err = endpoints.GetRepository(context.Background(), protocol.RepositoryRequest{Repo: repoForBroker(cfg)})
	return err
}
