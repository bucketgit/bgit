...
func TestResolveAWSSetupRegionUsesConfiguredRegion(t *testing.T) {
	...
	got, err := resolveAWSSetupRegion(context.Background(), internalsetup.Profile{Name: "prod", Region: "eu-west-1"}, "", false, strings.NewReader(""), ioDiscard{})
	if err != nil {
		t.Fatal(err)
	}
	if got != "eu-west-1" {
...
func TestResolveAWSSetupRegionPromptsFromEnabledRegions(t *testing.T) {
	...
	got, err := resolveAWSSetupRegion(context.Background(), internalsetup.Profile{Name: "prod"}, "", true, strings.NewReader("\x1b[B \x04"), &stdout)
	if err != nil {
		t.Fatal(err)
	}
	if got != "eu-west-1" {
...
func TestResolveAWSSetupRegionYesModeRequiresRegion(t *testing.T) {
	...
	writeFakeCLI(t, bin, "aws", []fakeCLIAction{})
	t.Setenv("PATH", bin+string(os.PathListSeparator)+os.Getenv("PATH"))
	errRegion, err := resolveAWSSetupRegion(context.Background(), internalsetup.Profile{Name: "prod"}, "", false, strings.NewReader(""), ioDiscard{})
...
func TestResolveAWSSetupRegionRequiresAWSCLI(t *testing.T) {
	...
	region, err := resolveAWSSetupRegion(context.Background(), internalsetup.Profile{Name: "prod", Region: "eu-west-1"}, "", true, strings.NewReader(""), ioDiscard{})
...
func TestResolveGCPSetupRegionUsesDialog(t *testing.T) {
	var stdout bytes.Buffer
	got, err := resolveGCPSetupRegion(internalsetup.Profile{Name: "work", Region: "us-central1"}, "", true, strings.NewReader("\x1b[B \x04"), &stdout)
	if err != nil {
		t.Fatal(err)
	}
	if got != "us-east1" {
...
func TestResolveGCPSetupRegionUsesExistingConfiguredRegion(t *testing.T) {
	var stdout bytes.Buffer
	got, err := resolveGCPSetupRegion(internalsetup.Profile{Name: "work", Region: "europe-west1", Existing: true}, "", true, strings.NewReader("\x04"), &stdout)
	if err != nil {
		t.Fatal(err)
	}
	if got != "europe-west1" {
...
func TestConfiguredSetupProfilesCarryConfiguredRegion(t *testing.T) {
	got := markConfiguredSetupProfiles([]internalsetup.Profile{{
		Provider: "gcs",
		Name:     "work",
		Region:   "us-central1",
	}}, internalconfig.Global{GCPProfiles: []internalconfig.GCPProfile{{
		Name: "work",
...
func TestConfiguredSetupBrokersSortedAndDetected(t *testing.T) {
	cfg := internalconfig.Global{
		GCPProfiles: []internalconfig.GCPProfile{{
			Name:      "work",
		...
		AWSProfiles: []internalconfig.AWSProfile{{
			Name:      "prod",
...
func TestSetupFormatsTeamTablesWithHeaders(t *testing.T) {
	members := setupFormatTeamMembers(protocol.Team{
		Name: "core",
		Members: []protocol.TeamMember{{
			Username: "owner",
			Role:     "admin",
		}},
	})
...
func TestSetupDialogPaginatesProfilesPerProvider(t *testing.T) {
	...
	for i := 0; i < 12; i++ {
		profiles = append(profiles, internalsetup.Profile{Provider: "gcs", Name: "gcp" + string(rune('a'+i))})
	}
	for i := 0; i < 11; i++ {
		profiles = append(profiles, internalsetup.Profile{Provider: "s3", Name: "aws" + string(rune('a'+i))})
	}
	rendered := renderSetupDialog(setupDialogState{
		profiles:         profiles,
...
func TestSetupCreateProfileDefaultsAvoidExistingDefault(t *testing.T) {
	defaults := setupCreateProfileDefaults([]internalsetup.Profile{{Provider: "s3", Name: "default"}}, setupOptions{})
	if defaults["s3"] != "" {
...
func TestSetupCommandProvisionsGCPAndWritesGlobalConfig(t *testing.T) {
	...
	var ownerReq protocol.OwnerRequest
...
func TestBrokerDeleteAWSDeletesStackAndClearsConfig(t *testing.T) {
	home := t.TempDir()
	configPath := filepath.Join(home, ".bgit", "config")
	if err := writeGlobalConfig(configPath, internalconfig.Global{
...
func TestSetupAvailableTeamUserChoicesGroupPendingAndExcludeMembers(t *testing.T) {
	choices := setupBrokerUserChoicesFromUsers([]protocol.UserInfo{{
		Username:   "owner",
		BrokerRole: "admin",
		Keys:       []protocol.Key{{PublicKey: "ssh-ed25519 AAAA owner"}},
	}, {
		Username:   "pending",
		BrokerRole: "user",
		Pending:    true,
		...
		Username:   "developer",
		BrokerRole: "user",
		Keys:       []protocol.Key{{PublicKey: "ssh-ed25519 AAAA dev"}},
...
func TestSetupBrokerOwnerUserOnlyShowsTransfer(t *testing.T) {
	choices := setupBrokerUserActionChoices(protocol.UserInfo{Username: "owner", BrokerRole: "owner", Keys: []protocol.Key{{PublicKey: "ssh-ed25519 AAAA owner"}}})
...
func TestSetupBrokerRegularUserShowsDelete(t *testing.T) {
	choices := setupBrokerUserActionChoices(protocol.UserInfo{Username: "ada", BrokerRole: "user", Keys: []protocol.Key{{PublicKey: "ssh-ed25519 AAAA ada"}}})
	var sawDelete bool
	for _, choice := range choices {
